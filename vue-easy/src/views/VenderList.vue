        ...
        <div class="col-md-12" v-for="{ venderName, venderIntroduce, writeOrUpdate, venderId, showphoto } in venders">
            <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                ...
                <div class="col p-4 d-flex flex-column position-static">
                    ...
                    <div class="mb-1 text-body-secondary">更新時間 :{{ writeOrUpdate.slice(0, 10) }}</div>
                    <p class="card-text mb-auto">{{ truncateText(venderIntroduce, 50) }}</p>
                    <button class="button-like" @click="clickHandler(venderId)" v-if="travelerid">
                        跟我聊聊
                    </button>
        ...
        <ul class="pagination pagination-sm">
            ...
            <li class="page-item" v-for="value, index in totalPages">
                <div class="btn-group me-2" role="group" aria-label="First group">
                    <button @click="changepage(index + 1)" class="btn btn-primary">{{ index + 1 }}</button>
                </div>
            </li>
        </ul>
    </nav>
